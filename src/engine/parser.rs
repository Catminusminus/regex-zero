//! Parse a regexp and transform it to an AST
use std::{
    error::Error,
    fmt::{self},
    mem::take,
...
// Type represents Parsing Errors
#[derive(Debug)]
pub enum ParseError {
    InvalidEscape(usize, char), // Invalid escape sequence
    InvalidRightParen(usize),   // No open paren
    NoPrev(usize),              // No expressions before +, |, *, ?
    NoRightParen,               // No closed paren
    Empty,                      // Empty Pattern
}

// Show readable Parse Errors for humans
...
/// Transform +, *, ? to AST
///
/// Error if no patterns before +, *, or ?
/// Ex: *ab, abc|* => Error
...
/// Transform multiple expressions conccated by Or to AST
///
/// Ex: abc|def|ghi => AST::Or("abc", AST::Or("def", "ghi"))
